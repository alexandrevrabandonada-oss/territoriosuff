...
O Monóxido de Carbono (CO) exige atenção metodológica especial devido à divergência de unidades e janelas de média regulamentares utilizadas pelo padrão brasileiro (CONAMA) e pela diretriz internacional (OMS). Esta auditoria confirma os critérios aplicados para o ano de 2024.

### 1.1. Unidade de Medida e Conversão Física
*   **Dados Originais:** Confirmado que a plataforma pública INEA/WebLakes fornece os dados horários brutos de CO na unidade de **ppm** (partes por milhão).
*   **Conversão para OMS:** A diretriz de qualidade do ar da OMS (2021) adota o limite diário de **4 mg/m³**. Para fins de comparação com a OMS, as médias diárias calculadas em ppm foram convertidas multiplicando-as pelo fator de conversão física de **1.145** (estabelecido para as condições de temperatura de 25°C e pressão de 1 atm).
*   **Limitação da Conversão:** A conversão física por `1.145` foi aplicada **exclusivamente** para a avaliação da régua diária da OMS. A comparação com a CONAMA seguiu a unidade nativa (ppm).

### 1.2. Média Móvel de 8 Horas e Representatividade
*   **Regra Regulatória da CONAMA:** O limite nacional da CONAMA 506/2024 estabelece o limite de **9 ppm** medido em médias móveis de 8 horas consecutivas.
*   **Algoritmo de Média Móvel:** Foi implementada uma janela deslizante horária de 8 horas. Para que uma janela seja representativa, exige-se o mínimo de **6 horas válidas** em cada janela de 8 horas. Janelas com menos de 6 leituras válidas foram descartadas do cálculo.
