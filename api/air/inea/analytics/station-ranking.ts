...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const stationMaxAqi: Record<string, { val: number; label: string }> = {};
    ...
    for (const m of measurements) {
      ...
      const aqiVal = m.air_quality_index || 0;

      // Max AQI (ignore insufficient data)
      if (validatedClass !== "DADO_INSUFICIENTE") {
        if (!stationMaxAqi[sId] || aqiVal > stationMaxAqi[sId].val) {
          stationMaxAqi[sId] = { val: aqiVal, label: validatedClass };
        }
      }
    ...
    const ranking = stations.map(station => {
      ...
      const maxAqiInfo = stationMaxAqi[station.id] || { val: 0, label: "BOA" };

      return {
        station_id: station.id,
        station_name: station.name,
        max_aqi: maxAqiInfo.val,
        max_aqi_classification: maxAqiInfo.label,
        measured_days,
        expected_days,
    ...
    // Sort by degraded percent of measured days descending, then max_aqi descending
    ranking.sort((a, b) => b.degraded_percent_of_measured_days - a.degraded_percent_of_measured_days || b.max_aqi - a.max_aqi);

    return res.status(200).json(ranking);
  } catch (err: any) {
    console.error("[api/air/inea/analytics/station-ranking] Error:", err.message);
