...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    // Grouping by station
    const stationTimestamps: Record<string, string[]> = {};
    ...
    for (const m of measurements) {
      ...
      if (!stationTimestamps[sId]) {
        stationTimestamps[sId] = [];
      }
      stationTimestamps[sId].push(m.measured_at);
    ...
    const gapThresholdMs = 24 * 60 * 60 * 1000; // 24 hours in milliseconds

    const results = stations.map(station => {
      const timestamps = stationTimestamps[station.id] || [];
      const actual_records = timestamps.length;
      ...
      // Calculate gaps > 24 hours
      let gap_count = 0;
      let max_gap_ms = 0;

      for (let i = 1; i < actual_records; i++) {
        const prevMs = new Date(timestamps[i - 1]).getTime();
        const currMs = new Date(timestamps[i]).getTime();
      ...
      const max_gap_hours = Math.round(max_gap_ms / (60 * 60 * 1000));
    ...
    console.error("[api/air/inea/analytics/data-gaps] Error:", err.message);
