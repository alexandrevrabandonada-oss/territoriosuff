  ...
  return (
    <div id="linha-tempo-plurianual" className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 border-b border-slate-100 pb-4">
        <div>
          <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2">
            ...
            Linha do Tempo Plurianual (2022–2024)
          </h3>
          <p className="text-slate-500 text-xs mt-1">
            Compare o histórico consolidado e as excedências de material particulado no Observatório do Ar.
          </p>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <SurfaceCard className="p-5 md:p-6 bg-white border border-slate-100 rounded-2xl space-y-5">
          <div>
            <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest block">Histórico de Exposição Crônica</span>
          ...
          <div className="space-y-4">
            {STATIONS.map((st) => {
              ...
              return (
                <div key={st.id} className={`space-y-1.5 p-2 rounded-xl transition-all ${isHighlighted ? 'bg-slate-50/80 border border-slate-200/40' : ''}`}>
                  <div className="flex justify-between items-center text-xs">
                    <span className="font-extrabold text-slate-700">{st.shortName}</span>
        ...
        {/* Card: Excedências Diárias */}
        <SurfaceCard className="p-5 md:p-6 bg-white border border-slate-100 rounded-2xl space-y-5">
          <div>
            <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest block">Eventos de Atenção Diários</span>
            <h4 className="text-sm font-bold text-slate-800 mt-1">Dias acima dos Padrões Recomendados ({selectedYear})</h4>
          </div>
          ...
          <div className="space-y-5">
                ...
                {STATIONS.map((st) => {
                  ...
                  return (
                    <div key={st.id} className="space-y-1">
                      <div className="flex justify-between items-center text-xs">
                        <span className="font-bold text-slate-600">{st.shortName}</span>
            ...
            <div className="space-y-3.5 pt-2">
              <div className="flex justify-between items-center text-xs border-b border-slate-100 pb-1">
                <span className="font-black text-slate-500 uppercase tracking-wider">Acima da Régua CONAMA 506 24h ({isPM10 ? ">50" : ">25"} {unit})</span>
              </div>
              <div className="space-y-3">
                {STATIONS.map((st) => {
                  const pData = yearSummary[st.id]?.pollutants[selectedPollutant];
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <SurfaceCard className={`p-4 border rounded-xl transition-all ${selectedStation === "69" ? 'border-amber-300 bg-amber-50/10' : 'border-slate-100 bg-white'}`}>
          ...
          <p className="text-slate-600 text-xs leading-relaxed">
            <strong>Belmonte concentra os maiores sinais de atenção.</strong> A estação registra de forma recorrente as médias anuais mais altas entre as três estações, liderando também as violações crônicas das réguas OMS e CONAMA nos períodos avaliados.
          </p>
        </SurfaceCard>
        ...
        <SurfaceCard className={`p-4 border rounded-xl transition-all ${selectedStation === "70" ? 'border-indigo-300 bg-indigo-50/10' : 'border-slate-100 bg-white'}`}>
          ...
          <p className="text-slate-600 text-xs leading-relaxed">
            <strong>Retiro aparece com episódios relevantes, especialmente em picos.</strong> Esta estação destaca-se por picos horários expressivos do material particulado no ano, somando múltiplos dias de atenção em termos de exposição diária e eventos críticos.
          </p>
        </SurfaceCard>
        ...
        <SurfaceCard className={`p-4 border rounded-xl transition-all ${selectedStation === "71" ? 'border-teal-300 bg-teal-50/10' : 'border-slate-100 bg-white'}`}>
          ...
          <p className="text-slate-600 text-xs leading-relaxed">
            <strong>Santa Cecília tem menores médias, mas não ausência de atenção.</strong> Embora registre historicamente os melhores índices comparativos do trio, ainda excede de modo frequente as diretrizes de proteção recomendadas pela OMS.
          </p>
        </SurfaceCard>
      ...
      <div className="bg-slate-50 border border-slate-150 p-4.5 rounded-2xl text-slate-500 text-[11px] leading-relaxed space-y-2.5">
        ...
        <div className="grid gap-2 sm:grid-cols-2 text-[10px]">
          ...
          <div>
            • <strong>Ausência de Dado Não é Ar Bom:</strong> Períodos com falha de transmissão ou lacuna instrumental não representam qualidade do ar em conformidade com as diretrizes.
          </div>
          <div>
            • <strong>Exposição Periódica:</strong> As leituras resumem a exposição histórica acumulada no período e não representam monitoramento minuto a minuto.
          </div>
        </div>
