...
function run() {
  console.log('Iniciando auditoria matemática de somas excedentes...');
  
  const results2025 = auditYear(2025);
  const results2026 = auditYear(2026);
  const allResults = [...results2025, ...results2026];

  let hasErrors = false;
  let reportMd = `# Estado da Nação — Auditoria de Somas de Excedências (2025-2026)

**Data da Auditoria:** ${new Date().toISOString().split('T')[0]}  
**Objetivo:** Verificar a consistência matemática entre as somas das ultrapassagens mensais e os totais consolidados anuais para os poluentes PM10 e PM2.5.  
...
`;
    ...
    reportMd += `> [!NOTE]
> **AUDITORIA APROVADA:** 100% dos cruzamentos matemáticos estão corretos. As somas mensais batem perfeitamente com os totais anuais/parciais informados nos sumários.
`;
  ...
  fs.writeFileSync(path.join(reportsDir, 'estado-da-nacao-observatorio-auditoria-somas-2025-2026.md'), reportMd, 'utf8');
  console.log(`Relatório salvo em reports/estado-da-nacao-observatorio-auditoria-somas-2025-2026.md`);
