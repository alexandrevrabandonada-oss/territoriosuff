...
async function auditAndRecomputeYear(year: number, isPartial: boolean) {
  ...
  const auditReportName = isPartial 
    ? 'estado-da-nacao-inea-cache-audit-2026-parcial-pm10-pm25.md'
    : 'estado-da-nacao-inea-cache-audit-2025-pm10-pm25.md';

  const recomputeReportName = isPartial
    ? 'estado-da-nacao-inea-recompute-pm10-pm25-2026-parcial.md'
    : 'estado-da-nacao-inea-recompute-pm10-pm25-2025.md';

  // 1. Generate Cache Audit Report
  let auditMd = `# Estado da Nação — Auditoria do Cache WebLakes ${year}${isPartial ? ' (Parcial)' : ''}
## Poluentes PM10 e PM2.5 nas Estações de Volta Redonda

**Data da Auditoria:** ${new Date().toISOString().split('T')[0]}  
**Fonte dos Dados:** Cache local raw de arquivos mensais JqGrid  
**Ano:** ${year}${isPartial ? ' (Parcial, Jan a Mai)' : ''}  
...
`;
  ...
  let recomputeMd = `# Estado da Nação — Recálculo Analítico de PM10 e PM2.5 — ${year}${isPartial ? ' (Parcial)' : ''}
...
2.  **Representatividade Anual:** Exige-se 75% de cobertura para validade das médias históricas consolidadas. ${isPartial ? 'Como 2026 é um ano parcial, os dados representam apenas a amostragem acumulada em andamento (janeiro a maio) e não devem ser interpretados como médias anuais finais.' : 'Os dados de 2025 atestaram cobertura excelente (>90% nas principais estações) e são estatisticamente válidos.'}
`;
...
async function run() {
  console.log('Starting Lote A Recomputation...');
  await auditAndRecomputeYear(2025, false);
  await auditAndRecomputeYear(2026, true);
  console.log('Lote A Recomputation Complete!');
}
